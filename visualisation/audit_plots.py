...
def figure_1_feature_importance() -> None:
    """Single panel. Top-10 features from the 3-month GBR's `feature_importances_`
    attribute, with the numeric value annotated on each bar."""
    res = get_results()[("3m", True, "GBR")]
    ...
    ax.set_title("Top-10 features by importance — GBR, 3-month horizon")
    ax.tick_params(axis="y", labelsize=9)

    fig.suptitle("Feature importance values from `model.feature_importances_` (sum to 1.0)", y=1.02, fontsize=11)
    plt.tight_layout()
...
def figure_2_model_comparison() -> None:
    """2×2 grid at the 3-month horizon. Rows = no/with players_7days_after_release.
    Cols = R² / MAE. Bars = LR / RF / GBR.
    """
    results = get_results()
    horizon = "3m"
    models = ["LR", "RF", "GBR"]
    ...
    bar_colors = ["#9ec5e8", "#4d8fd1", "#1f4e79"]
    ...
    fig.suptitle("Model comparison at the 3-month horizon", y=1.01, fontsize=12, fontweight="bold")
    plt.tight_layout()
...
def figure_3_pred_vs_actual() -> None:
    """Predicted vs actual CCU at the 3-month horizon, log-log scale, GBR with feature."""
    res = get_results()[("3m", True, "GBR")]
    ...
    ax.scatter(y_true_raw[mask], y_pred_raw[mask], alpha=0.4, s=14, color="steelblue")
    if mask.sum() > 0:
        ...
        ax.set_xscale("log")
        ax.set_yscale("log")
    ax.set_xlabel("Actual CCU at 3 months")
    ax.set_ylabel("Predicted CCU at 3 months")
    ax.set_title(f"Predicted vs actual CCU — 3-month horizon (R² = {res['r2']:.3f})")
...
def figure_4_residuals() -> None:
    """Residuals (observed − predicted, log-CCU) vs predicted (log-CCU) at the 3-month horizon."""
    res = get_results()[("3m", True, "GBR")]
    residuals = res["y_test"] - res["y_pred"]

    fig, ax = plt.subplots(figsize=(9, 6))
    ax.scatter(res["y_pred"], residuals, alpha=0.4, s=14, color="steelblue")
    ax.axhline(0, color="red", linestyle="--", linewidth=1.5)
    ax.set_xlabel("Predicted (log-CCU)")
    ax.set_ylabel("Residual (observed − predicted, log-CCU)")
    ax.set_title("Residuals vs predicted — 3-month horizon")
...
def figure_5_week1_vs_target() -> None:
    """1×2 grid at the 3-month horizon. Cols = linear / log scale.
    Each panel: scatter of players_7days_after_release vs 3-month target + linear fit.
    """
    rel_path, target_col = HORIZONS["3m"]
    df = pd.read_csv(DATA_DIR / rel_path)
    df.columns = df.columns.str.strip().str.replace("﻿", "")
    ...
    for col, scale in enumerate(["linear", "log"]):
        ...
        if scale == "log":
            ...
            if mask.sum() > 1:
                ...
                ax.plot(xline, yline, "r-", linewidth=1.7, label=f"log-fit: slope = {slope:.2f}")
            ax.set_xscale("log")
            ax.set_yscale("log")
        else:
            if len(x) > 1:
                ...
                ax.plot(xline, yline, "r-", linewidth=1.7,
                        label=f"linear-fit: y = {slope:.2f}x + {intercept:.0f}")
        ax.set_xlabel(f"{WEEK1_COL} (day-7 CCU snapshot)")
        ax.set_ylabel(target_col)
        ax.set_title(f"3-month horizon ({scale} scale)")
        ax.legend(fontsize=10)
        ax.grid(True, which="both", alpha=0.3)

    fig.suptitle("Day-7 CCU vs 3-month CCU", y=1.02, fontsize=12, fontweight="bold")
    plt.tight_layout()
